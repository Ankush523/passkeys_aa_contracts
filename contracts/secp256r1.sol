...
contract secp256r1{
    ...
    function Verify(PassKeyId memory passKey, uint r, uint s, uint e) internal view returns(bool)
    {
        ...
        JacobianPoint[16] memory p =_preComputeJacobianPoints(passKey);
        return verifyPrecompute(p, r, s, e);
    }

    function verifyPrecompute(JacobianPoint[16] memory points, uint r, uint s, uint e) internal view returns(bool)
    {
        if (r >= nn || s >= nn) {
            return false;
        }
        ...
        (x,y) = ScalarMultiplication(points, u1, u2);
        return (x==r);
    }

    function ScalarMultiplication(JacobianPoint[16] memory points,uint u1, uint u2) internal view returns(uint,uint)
    {
        uint x = 0;
        uint y = 0;
        uint z = 0;
    ...
    function _preComputeJacobianPoints(PassKeyId memory passKey)internal pure returns (JacobianPoint[16] memory points)
    {
        ...
        points[4] = JacobianPoint(gx, gy, 1); // u1Points[1]
    ...
    function _jPointAdd(JacobianPoint memory point1,JacobianPoint memory point2) internal pure returns (JacobianPoint memory)
    {
        uint x;
        uint y;
        uint z;
        (x, y, z) = _jAdd(point1.x, point1.y, point1.z, point2.x, point2.y, point2.z);
        return JacobianPoint(x, y, z);
    }

    function _jPointDouble(JacobianPoint memory points) internal pure returns(JacobianPoint memory)
    {
        uint x;
        uint y;
        uint z;
        (x, y, z) = _modifiedJacobianDouble(points.x, points.y, points.z);
        return JacobianPoint(x, y, z);
    }
    ...
    function _jAdd(uint p1, uint p2, uint p3, uint q1, uint q2, uint q3) internal pure returns(uint r1, uint r2, uint r3)
    {
        if (p3 == 0) {
            r1 = q1;
            r2 = q2;
    ...
    function _invmod(uint value, uint p) internal view returns (uint ret)
    {
